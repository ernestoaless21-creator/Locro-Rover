...
function loadPreview() {
  ...
  router.get(
    route('purchases.import', { team: props.team }),
...
const existingProductIds = computed(() => new Set(props.sourceData?.existing_product_ids ?? []))
...
function resetSelection() {
  ...
  for (const item of sourceItems.value) {
    // Los productos que ya están en el destino arrancan destildados para no
    // reintentar una duplicación que el backend igual va a rechazar.
    state[item.product_id] = !existingProductIds.value.has(item.product_id)
  }
  checked.value = state
}
...
function submit() {
  ...
  form.selected_product_ids = ids
  form.post(route('purchases.import.store', { team: props.team }))
}
</script>

<template>
  <AppLayout title="Importar planificación de compras">
    <template #header>
      <div class="flex items-center gap-4">
        <a :href="route('purchases.index', { team })" class="text-xs text-indigo-600 hover:text-indigo-800 uppercase tracking-wide">
      ...
      <div class="max-w-2xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
          <p class="text-sm text-gray-600">
            Importar productos planificados de una edición anterior como base para esta edición.
            Se copian categoría, unidad, cantidades de referencia (1000/1500 porciones), precio estimado y
            proveedor previsto. No se copian cantidad real comprada, precio real ni proveedor real.
            Los productos que ya están en esta edición se omiten para evitar duplicados.
          </p>
          ...
          <div v-if="sourceItems.length > 0 && !sameYear" class="space-y-2">
            <p class="text-sm font-medium text-gray-700">Elegí qué productos importar</p>
            <div class="rounded-lg border border-gray-200 divide-y divide-gray-100 max-h-96 overflow-y-auto">
              <label
                ...
                :class="existingProductIds.has(item.product_id) ? 'opacity-50' : ''"
              >
                <input type="checkbox" v-model="checked[item.product_id]" class="rounded text-indigo-600 focus:ring-indigo-500" />
                <span class="flex-1 text-sm text-gray-700">
                  <span v-if="item.category_name" class="text-gray-400">{{ item.category_name }} — </span>
                  {{ item.product_name }}
                </span>
                <span class="text-xs text-gray-400 font-mono">
                  <template v-if="item.qty_1000">1000p: {{ item.qty_1000 }}</template>
                  <template v-if="item.qty_1000 && item.qty_1500"> · </template>
                  <template v-if="item.qty_1500">1500p: {{ item.qty_1500 }}</template>
                </span>
                <span v-if="existingProductIds.has(item.product_id)" class="text-xs text-amber-600 whitespace-nowrap">
                  ya existe
                </span>
              </label>
            </div>
          ...
          <p v-else-if="sourceData && sourceItems.length === 0 && !sameYear" class="text-sm text-amber-700">
            La edición origen no tiene productos planificados.
          </p>
          ...
          <div class="flex gap-3 pt-2">
            ...
            <a :href="route('purchases.index', { team })" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">
              Volver
            </a>
          </div>
        </div>
